class Resize {
    ...
    convertImage(file, maxWidth, maxHeight, compressFormat, quality, rotation, responseUriFunc, outputType) {
        return Resize.createResizedImage(file, maxWidth, maxHeight, compressFormat, quality, rotation, responseUriFunc, outputType)        
    }
        ...
        if (file) {
            ...
            reader.onload = () => {
                ...
                image.onload = function () {
                    var resizedDataUrl = Resize.resizeAndRotateImage(image, maxWidth, maxHeight, compressFormat, quality, rotation);
                    blob = Resize.b64toBlob(resizedDataUrl, `image/${compressFormat}`);
                    let imgOutput = {
                        blobImg: blob,
                        base64Img: resizedDataUrl
                    }
                    outputType = responseUriFunc(imgOutput)
                    // outputType === 'blob' ?
